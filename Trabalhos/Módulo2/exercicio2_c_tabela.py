...
# A nossa fiel fonte de símbolos
...
def carregar_lista(caminho):
    try:
        # Tenta ler com a codificação moderna (UTF-8)
        with open(caminho, 'r', encoding='utf-8') as f:
            return [linha.strip() for linha in f if linha.strip()]
    except UnicodeDecodeError:
        # Se falhar nos acentos, tenta com a codificação do Windows (Latin-1)
...
def gerar_tabela_pessoas(numero_ficheiro, quantidade=1000):
    # 1. Carregar os dados dos ficheiros do professor
    # Ajusta o nome da pasta abaixo se lhes deste outro nome
    ...
    # 2. Preparar as probabilidades equiprováveis para Nomes, Localidades, etc.
    ...
    # 3. Preparar o Alfabeto e Probabilidades para o ID
    # Primeiro dígito: Lei de Benford (1 a 9)
    alfabeto_benford = [str(d) for d in range(1, 10)]
    prob_benford = [math.log10(1 + 1/d) for d in range(1, 10)]
    
    # Restantes 7 dígitos: Equiprováveis (0 a 9)
    ...
    with open(nome_ficheiro, 'w', encoding='utf-8') as f:
        # Cabeçalho da tabela (formato CSV separado por ponto e vírgula)
        ...
        for _ in range(quantidade):
            ...
            # Gerar Nome (Primeiro Nome + Apelido)
...
# O professor pede 3 ficheiros com pelo menos 1000 linhas
for i in range(1, 4):
